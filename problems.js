...
const PROBLEMS = [
...

  {
    ...
    difficulty: "Medium",
    prompt: `<p>How many people need to be in a room before it's <em>more likely than not</em> that at least two of them share a birthday? Most people's gut says something near 183 (half of 365). Work out whether that's right — and if not, why the true number is so different.</p>`,
    hints: [
...

  {
    ...
    category: "Biology",
    difficulty: "Medium",
    prompt: `<p>A disease affects <strong>1 in 10,000</strong> people. A test is 99% accurate (99% sensitivity, 99% specificity). You test <em>positive</em>. What's the probability you actually have the disease? (Your gut may say around 99% — work out whether that's right.)</p>`,
...

  {
    ...
    solution: `<p>Start both hourglasses together at <code>t = 0</code>.</p>
      <ul>
        <li><code>t = 4</code>: the 4-min glass empties — flip it (it restarts).</li>
        <li><code>t = 7</code>: the 7-min glass empties — flip it (it restarts). The 4-min glass, running since t=4, now has 1 minute of sand left in its top.</li>
        <li><code>t = 8</code>: the 4-min glass empties (that last minute ran out). The 7-min glass has been running 1 minute since t=7, so 1 minute of sand now sits in its bottom. Flip the 7-min glass once more.</li>
        <li><code>t = 9</code>: that 1 minute of sand runs out — exactly 9 minutes measured.</li>
      </ul>`,
...

  {
    ...
    category: "Logic",
    difficulty: "Medium",
    prompt: `<p>Three jars are labeled <strong>Apples</strong>, <strong>Oranges</strong>, and <strong>Apples &amp; Oranges</strong>. You're told that <em>every</em> label is wrong. Without looking inside, you may reach into just <strong>one</strong> jar and pull out a single fruit. How can that one draw let you correctly relabel all three jars?</p>`,
    hints: [
      "Use the rule that EVERY label is wrong — it's a strong constraint. What does it force about the jar labeled 'Apples & Oranges'?",
      "That jar can't actually be the mixed one, so it must be pure. Draw from it — then let 'all labels are wrong' cascade to the other two by elimination."
    ],
    solution: `<p>Draw from the jar labeled <strong>Apples &amp; Oranges</strong>. Because its label is wrong, it isn't mixed — it's pure — so the single fruit you pull reveals its true contents. Say you draw an apple: this jar is really <em>Apples</em>. Now the jar labeled <em>Oranges</em> can't be Oranges (label wrong) and can't be Apples (already identified), so it must be the <em>Apples &amp; Oranges</em> jar. That leaves the jar labeled <em>Apples</em> as the <em>Oranges</em> jar. One draw settles all three.</p>`,
    probe: "Which jar is most constrained by 'all labels are wrong'? — the mislabeled 'mixed' jar must be pure, so one draw fully determines it and the rest follow by elimination.",
    lesson: "When every option is constrained, start where the constraint bites hardest and let deduction cascade. Resolving the most-constrained variable first is exactly how constraint-satisfaction solvers and Sudoku strategies work."
  },

  {
    id: "wine-water",
    title: "The wine and the water",
    category: "Puzzles",
    difficulty: "Medium",
    prompt: `<p>You have a glass of red wine and a glass of water, each holding the same amount. You take one spoonful of wine and stir it into the water; then you take one spoonful of that (now slightly wine-tinted) mixture and stir it back into the wine. Is there now <strong>more wine in the water glass, or more water in the wine glass</strong>?</p>`,
    hints: [
      "Don't chase the messy concentrations — track the totals. After the two transfers, how does the amount of liquid in each glass compare to the start?",
      "Each glass ends with exactly the volume it began with. So whatever wine is now missing from the wine glass must have been replaced by an equal volume of something — what?"
    ],
    solution: `<p>They are exactly <strong>equal</strong>. Each glass starts and ends with the same total volume, so the wine glass finishes holding as much liquid as it began with. Whatever volume of wine now sits in the water glass must have been replaced, in the wine glass, by exactly that same volume of water — otherwise the volumes wouldn't balance. So the wine in the water equals the water in the wine, no matter the spoon size, how well you stir, or how many times you transfer back and forth.</p>`,
    probe: "Instead of tracking concentrations, ask 'what is conserved?' — each glass returns to its original volume, which forces the two contaminations to be equal.",
    lesson: "A conservation argument (the volumes must balance) settles a problem that looks hopeless if you try to simulate the mixing. Hunting for the invariant instead of tracking details is a recurring shortcut in physics, chemistry, and accounting."
  },

  {
    id: "monk-mountain",
    title: "The monk on the mountain",
    category: "Logic",
    difficulty: "Medium",
    prompt: `<p>A monk leaves the base of a mountain at sunrise, walks the single narrow path to the top, and arrives by sunset. The next morning he leaves the top at sunrise and walks the same path back down, arriving at the base by sunset — strolling, resting, and changing pace freely on both days. Must there be a spot on the path he occupies at <strong>exactly the same time of day</strong> on both days?</p>`,
    hints: [
      "Reasoning about his speeds gets messy. Instead, picture both journeys happening on the SAME day — what are you really comparing?",
      "Imagine two monks on one day: one walking up, one walking down, each starting at sunrise on the same path. What is forced to happen to them?"
    ],
    solution: `<p><strong>Yes</strong>, necessarily. Superimpose the two days: imagine one monk walking up and another walking down the <em>same</em> path on the <em>same</em> day, both starting at sunrise and finishing by sunset. Since they traverse the same one-dimensional path in opposite directions over the same time window, they must pass each other at some instant — and at that instant they occupy the same place at the same time of day. Translating back to the real monk, that meeting point is a single spot he is at during the identical time on both days. (Formally it's the Intermediate Value Theorem: the difference between his day-1 and day-2 positions starts positive and ends negative, so it must cross zero.)</p>`,
    probe: "Can I overlay the two separate journeys into one scene where the answer becomes a meeting? — two travelers on the same path going opposite ways must cross.",
    lesson: "Recasting two separate processes as one simultaneous scene turns an abstract question into an obvious collision. This 'superposition' move — and the Intermediate Value Theorem behind it — proves that something exists without ever pinning down where or when."
  },

  {
    id: "fly-between-trains",
    title: "The fly between two trains",
    category: "Puzzles",
    difficulty: "Medium",
    prompt: `<p>Two trains are 100 km apart on a straight track, heading toward each other at 50 km/h each. A fly starts at the nose of one train and flies at 75 km/h to the other train, instantly turns around, flies back, and keeps bouncing between them until the trains meet. What <strong>total distance</strong> does the fly travel?</p>`,
    hints: [
      "You could sum the infinitely many shorter and shorter legs — but there's a one-line route. What quantity is easy to pin down first?",
      "The fly flies at a constant speed the whole time, so its distance is just speed × time. How long until the trains meet?"
    ],
    solution: `<p><strong>75 km.</strong> The trick is to find the <em>time</em>, not sum the bounces. The two trains close a 100 km gap at a combined 100 km/h, so they meet after exactly 1 hour. The fly is airborne that entire hour at a constant 75 km/h, so it covers <code>75 × 1 = 75</code> km. (Legend says von Neumann, asked this, instantly summed the infinite series instead — getting 75 the hard way.)</p>`,
    probe: "Is there a simpler quantity I can compute first that makes the hard sum unnecessary? — find how long the fly flies, then distance is just speed × time.",
    lesson: "Change the variable you solve for. What is an intractable infinite sum in one framing (leg-by-leg distance) is trivial in another (total time). Asking 'what's the easiest thing to compute here?' often dissolves the hard part."
  },

  {
    id: "cheese-cube",
    title: "Cutting the cheese cube",
    category: "Puzzles",
    difficulty: "Medium",
    prompt: `<p>You have a 3×3×3 cube of cheese and want to cut it into 27 unit cubes. Between cuts you may rearrange the pieces however you like, but each cut is a single straight planar slice. What is the <strong>minimum number of cuts</strong>, and why can't rearranging beat it?</p>`,
    hints: [
      "Rearranging clearly helps most pieces. So focus on one very special piece: the little cube at the exact center of the 3×3×3 block.",
      "The center cube's 6 faces are all interior to the original block. Each must be created by some cut — can one straight slice create more than one of them?"
    ],
    solution: `<p><strong>6 cuts.</strong> Rearranging does help many pieces, but look at the small cube at the very center of the block. Each of its 6 faces lies in the interior of the original cube, so every one must be produced by a cut — and a single planar slice can create at most one of them (those six faces point in six different directions, and one plane can form only one). So at least 6 cuts are required, whatever the rearranging. And 6 (two per axis) obviously suffice.</p>`,
    probe: "Is there one piece whose own structure forces a lower bound? — the center cube's six interior faces each demand a separate cut, so no rearrangement beats 6.",
    lesson: "To prove a minimum, find the single element that forces it — here the center cube's six faces. Reasoning about the most-constrained sub-object yields lower bounds that clever global tricks can't get around, a staple of combinatorics."
  },

  {
    ...
    category: "Puzzles",
    difficulty: "Easy",
    prompt: `<p>A snail sits at the bottom of a 30-foot well. Each day it climbs up 3 feet, but each night while it sleeps it slips back down 2 feet. How many days does it take the snail to climb out of the well?</p>`,
    hints: [
      "The tempting 'net 1 foot per day, so 30 days' is a trap. What is different about the very last day?",
      "On the day the snail's climb reaches the top, it's already out and doesn't slip back that night. Where does it stand at the start of each day?"
    ],
    solution: `<p><strong>28 days.</strong> The snail nets 1 foot per full day-and-night (up 3, down 2), but on the day it crests the top it escapes before the nightly slip. At the start of day <code>k</code> it is at <code>k − 1</code> feet; during the day it climbs 3 to reach <code>k + 2</code> feet, escaping as soon as that hits 30. So <code>k + 2 ≥ 30 → k = 28</code>: on day 28 it starts at 27 feet, climbs to 30, and is out — no slip. (Not 30 days: the final climb has no night.)</p>`,
    probe: "Does the repeating pattern still hold on the last step? — the nightly slip stops once the snail is out, so treat the final climb separately from the steady net rate.",
    lesson: "Watch the boundary of a repeating process — the last step often breaks the pattern. Averaging a per-cycle rate over the whole journey overcounts when the goal is reached mid-cycle. This 'off-by-one at the finish' recurs in loops, scheduling, and rate problems."
  },

  {
    ...
    category: "Puzzles",
    difficulty: "Easy",
    prompt: `<p>Why are manhole covers round? This is a classic interview question — the interviewer wants your <em>reasoning</em>, not a memorized line. What advantages does a round cover have that a square or rectangular one doesn't?</p>`,
    hints: [
      "Think about failure modes of the cover itself. What's the worst thing a heavy steel cover could do to the workers down below?",
      "Compare a circle to a square: can a square cover be turned and dropped through its own hole? What property of the circle rules that out?"
    ],
    solution: `<p>The standout reason: a round cover <strong>can't fall through its own hole</strong>. A square cover can be turned diagonally and slipped through the opening (a square's diagonal is longer than its side), which could drop it onto workers below; a circle has <em>constant width</em>, so in every orientation it's too wide to fit through. Other solid answers: a round cover needs no rotating to line up with the hole, it can be rolled to move it, it uses the least material for a given opening, and it distributes stress evenly with no weak corners. A strong response leads with the constant-width safety point, then adds a couple more — showing you reason from first principles rather than reciting.</p>`,
    probe: "What could go wrong with this object, and which shape eliminates that failure? — a circle's constant width is the one shape that can't fall through its own hole.",
    lesson: "Open-ended 'why' questions reward structured reasoning from failure modes and properties, not one memorized answer. Enumerate what could go wrong, then find the design that rules it out — the habit behind good engineering and design interviews."
  }
];
